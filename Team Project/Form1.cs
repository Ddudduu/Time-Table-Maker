...
namespace Team_Project
{
    public partial class Form1 : Form
    {
        ...
        const int btn_x = 70; //시간표 시작 위치 x좌표
        const int btn_y = 110; //시간표 시작 위치 y좌표

        const int btn_col = 6; //시간표 열 개수
        const int btn_row = 10; //시간표 행 개수

        const int lvw_width = 37; //리스트뷰 학점 등 너비
        const int lvw_prof_width = 49; //리스트뷰 교수명 너비

        const int color_num = 11;//색개수

        int filled_day = 5;//채워진 요일
        int filled_period = 6;//채워진 교시

        Button[] btn; //시간표 구성할 버튼배열

        int credit;//현재 학점
        int max_credit = 0;       // 최대학점
        ...
        int[] c = Enumerable.Repeat<int>(0, color_num).ToArray<int>();
        ...
        private void lvw_Subject_Init()
        {
            ...
            //칼럼별 너비 지정
            lvw_Subject.Columns[1].Width = lvw_width * 2; 
            lvw_Subject.Columns[2].Width = lvw_width; 
            lvw_Subject.Columns[3].Width = lvw_width;
            lvw_Subject.Columns[4].Width = lvw_width;
            lvw_Subject.Columns[5].Width = lvw_width;
            lvw_Subject.Columns[6].Width = lvw_prof_width;
            lvw_Subject.Columns[7].Width = lvw_width;
            lvw_Subject.Columns[8].Width = lvw_width;
            lvw_Subject.Columns[9].Width = lvw_width;
            lvw_Subject.Columns[10].Width = lvw_width;
        ...
        private void button(object sender, EventArgs e) 
        {
            ...
            btn = new Button[btn_row * btn_col];

            for (int i = 0; i < btn_row * btn_col; i++)
            {
                btn[i] = new Button();
                btn[i].Parent = this;
                btn[i].Text = "";
        ...
        private void button_Click(object sender, EventArgs e)       
        {
            ...
            { MessageBox.Show("이미 과목을 추가한 시간대입니다."); }      /* 이미 과목이 추가된 버튼 누를 경우 */
        }
        ...
        private void lvw_Subject_DoubleClick(object sender, EventArgs e) 
        {
            ...
            string[] sub_info = new string[lvw_Subject.Columns.Count-1];
            ListViewItem l = lvw_Subject.FocusedItem;

            for (int i = 0; i < lvw_Subject.Columns.Count-1; i++)
            {
                sub_info[i] = l.SubItems[i].Text;
                item += l.SubItems[i].Text;
                item += " ";
            }

            int sub_num = sub_info[0].Length-2;
            ...
            {
                char[] num = new char[sub_num];
                num = sub_info[0].Substring(2, sub_num).ToCharArray();
                int[] text = new int[sub_num / 2];
                int j = 0;
                int ind = 0;
                ...
                while (j < sub_num)
                {
                    ...
                    //10교시 처리
                    if (j+2< sub_num && (int)num[j + 1] == 49 && (int)num[j + 2] == 48)
                        m = 58-49;
                    if (m+1 > cur_period)
                        cur_period = m+1;
                    if (o+1 > cur_day)
                ...
                {
                    int nn = text[i];
                    if (btn[nn].Text != "" && btn[nn].Text != "XXX")        
                    {
                ...
                for (int i = 0; i < sub_num / 2; i++)
                {
                    ...
                    if (cur_day > filled_day)
                        filled_day = cur_day;

                    if (cur_period > filled_period)
                        filled_period = cur_period;
                }
                item += ci.ToString();
            ...
            //이수 학점 설정 
            txtbox_Credit.Text = (int.Parse(txtbox_Credit.Text) + int.Parse(sub_info[4])).ToString();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                int order = int.Parse(item.Substring(item.Length - 1, 1));
                
                c[order] =0;
                string[] it = item.Split(' ');               

                int sub_num = it[0].Length-1;
                char[] num = new char[sub_num];
                num = item.Substring(1, sub_num).ToCharArray();
                int n;
                int j = 1;
                while (j < sub_num)
                {
                    ...
                    int m = (int)num[j + 1]-49;
                    int o = (int)num[j] - 65;
                    if (j + 2 < sub_num && (int)num[j + 1] == 49 && (int)num[j + 2] == 48)//10교시 처리
                        m = 58-49;
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            int height = (filled_period - 10) * b_h2;
            int width = (filled_day - 6)*btn_width;
        ...
        */
        ...
        {
            max_credit = Convert.ToInt32(cmbBox_Max_Credit.SelectedItem.ToString());
        }
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            //c배열(색) 초기화
            c = Enumerable.Repeat<int>(0, color_num).ToArray<int>();
